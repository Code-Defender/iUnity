...
class _VoiceScreenState extends State<VoiceScreen> {
  ...
  // Local state to track "joined" voids for trending cards
  final Set<String> _joinedVoids = {};
  
  // Local state to track locally verified report IDs (to prevent double voting/immediate update feedback)
  final Set<String> _locallyVerifiedReports = {};
  final Map<String, int> _localVerificationIncrements = {};
  ...
  // Mock trending concerns data
  final List<Map<String, dynamic>> _trendingConcerns = [
    {
      "id": "trend_1",
      "tag": "Critical",
      "change": "24h Change: +14%",
      "title": "Fuel Surge: I-80",
      "description": "Localized price fixing identified at Wyoming corridor hubs.",
      "initialNodes": 128,
    },
    {
      "id": "trend_2",
      ...
      "title": "Dispatch Ghosting",
      "description": "Tier 2 brokers failing to respond after detention claims.",
      "initialNodes": 84,
    },
    {
      "id": "trend_3",
      ...
      "change": "Developing",
      "title": "Wait Times: NJ",
      "description": "Port Elizabeth terminal 4 reporting 6hr average turn times.",
      "initialNodes": 42,
    }
  ];

  // Predefined mockup reports to merge with live Firestore database reports
  final List<Map<String, dynamic>> _mockReports = [
    {
      ...
      "severity": "HIGH",
      "reportsCount": 214,
      "title": "Fuel Pricing Anomaly",
      "description": "Identified 45% markup above market average at three independent stops within 50 miles. Coordinated pattern suspected.",
    },
    {
      "id": "mock_report_2",
      "region": "NATIONAL",
      "severity": "MID",
      "reportsCount": 56,
      "title": "Dispatch Abuse",
      "description": "Load board ghosting during ETA verification. Brokers canceling without penalty after arrival confirmed by ELD.",
    },
    {
      "id": "mock_report_3",
      "region": "PACIFIC",
      "severity": "LOW",
      "reportsCount": 312,
      "title": "Terminal Access Delay",
      "description": "Persistent gate failure at LA Basin port terminals causing unpaid detention exceeding safety thresholds.",
    }
  ];

  void _toggleJoinVoid(String id, String title) {
    setState(() {
      if (_joinedVoids.contains(id)) {
        _joinedVoids.remove(id);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Disconnected from $title Void."),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Signal aligned. Joined $title Void."),
  ...
  void _verifySignal(String reportId, bool isMock, {DocumentReference? docRef}) async {
    final user = _authService.currentUser;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Authentication required to verify signals."),
  ...
  void _showReportDialog() {
    showGeneralDialog(
      ...
      pageBuilder: (context, anim1, anim2) {
        return _ReportPatternDialog(
          onReportSubmitted: () {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text("Report broadcasted successfully to the iUnity network!"),
                backgroundColor: AppColors.primaryContainer,
              ),
            );
          },
  ...
  Color _getTagBgColor(String tag, ThemeData theme) {
    switch (tag.toUpperCase()) {
      ...
      default:
        return AppColors.border.withOpacity(0.15);
    }
  }

  Color _getTagTextColor(String tag, ThemeData theme) {
    switch (tag.toUpperCase()) {
      case "CRITICAL":
      case "HIGH":
  ...
  Widget _buildHeroSection(ThemeData theme, bool isDesktop) {
    return Container(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        "Collective intelligence monitoring patterns of injustice. Every report strengthens the network and exposes systemic friction.",
                  ...
                  children: [
                    ...
                    OutlinedButton(
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Every shared voice adds nodes to our real-time truth map."),
                            backgroundColor: AppColors.surfaceContainerHigh,
                          ),
                        );
                      },
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              final item = _trendingConcerns[index];
              final id = item["id"] as String;
              final isJoined = _joinedVoids.contains(id);
              final activeNodesCount = item["initialNodes"] + (isJoined ? 1 : 0);
  ...
  Widget _buildLiveStreamSection(ThemeData theme, bool isDesktop) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Live stream header
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                IconButton(
                  icon: const Icon(Icons.filter_list, color: AppColors.onSurfaceMuted, size: 20),
                  onPressed: () {
                    // Reset filters
                    setState(() {
  ...
  Widget _buildDesktopGrid(
    ...
    // Collect list of widgets to display in grid
    List<Widget> streamCards = [];
    for (int idx in filteredIndices) {
      streamCards.add(
        _buildVoiceCard(context, theme, allReports[idx], docRefs[idx], isMocks[idx]),
      );
    }
    ...
    for (int idx in filteredIndices) {
      children.add(
        Padding(
          ...
          child: _buildVoiceCard(context, theme, allReports[idx], docRefs[idx], isMocks[idx]),
        ),
      );
    }
  ...
  Widget _buildVoiceCard(
    ...
    final String region = report['region'] ?? 'NATIONAL';
    final String severity = report['severity'] ?? 'MID';
    final String title = report['title'] ?? 'Pattern Report';
  ...
  Widget _buildBentoInsightCard(ThemeData theme) {
    return Container(
      constraints: const BoxConstraints(minHeight: 320),
      child: Stack(
        children: [
          // Gradient Background styling (Bento Look)
          Positioned.fill(
            child: ClipRRect(
          ...
          GlassCard(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                Text(
                  "Dispatch Pattern Correlated with Port Congestion",
                  style: theme.textTheme.headlineMedium?.copyWith(
                    fontWeight: FontWeight.w700,
                    fontSize: 22,
                    letterSpacing: -0.5,
                ...
                const SizedBox(height: 12),
                Text(
                  "Our collective intelligence has linked the rise in dispatch ghosting specifically to terminals with current turn-times exceeding 5 hours. Brokers are preemptively dropping loads to avoid detention payouts.",
                  style: theme.textTheme.bodyMedium?.copyWith(
                    fontSize: 14,
                    height: 1.6,
                    color: AppColors.onSurfaceMuted,
                  ...
                  children: [
                    InkWell(
                      onTap: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Findings copied to secure clipboard."),
                            backgroundColor: AppColors.primaryContainer,
                          ),
                        );
                      },
                      child: Row(
                        children: [
                          const Icon(Icons.share, color: AppColors.primary, size: 16),
                          const SizedBox(width: 8),
                          Text(
                            "Share Findings",
                            style: GoogleFonts.jetBrainsMono(
                    ...
                    InkWell(
                      onTap: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Drafting Collective Arbitration petition..."),
                            backgroundColor: AppColors.primaryContainer,
                          ),
                        );
                      },
      ...
      child: GlassCard(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            Text(
              "Drivers contributing to the real-time truth graph across North America.",
...
// Dialog form for creating a new report pattern
class _ReportPatternDialog extends StatefulWidget {
  final VoidCallback onReportSubmitted;

  const _ReportPatternDialog({required this.onReportSubmitted});

  @override
  State<_ReportPatternDialog> createState() => _ReportPatternDialogState();
}

class _ReportPatternDialogState extends State<_ReportPatternDialog> {
  ...
  String _category = "Fuel Pricing";
  String _region = "Southwest";
